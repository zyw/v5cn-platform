package cn.v5cn.platform.framework.ip.core.utils;

import cn.hutool.core.io.resource.ResourceUtil;
import cn.hutool.core.lang.Assert;
import cn.hutool.core.text.csv.CsvRow;
import cn.hutool.core.text.csv.CsvUtil;
import cn.v5cn.platform.framework.core.utils.ObjectUtils;
import cn.v5cn.platform.framework.core.utils.StreamUtils;
import cn.v5cn.platform.framework.ip.core.Area;
import cn.v5cn.platform.framework.ip.core.enums.AreaTypeEnum;
...
public class AreaUtils {
    ...
    public static Area parseArea(String pathStr) {
        ...
        for (String path : paths) {
            if (area == null) {
                area = StreamUtils.findFirst(areas.values(), item -> item.getName().equals(path));
            } else {
                area = StreamUtils.findFirst(area.getChildren(), item -> item.getName().equals(path));
            }
        }
        return area;
    }
    ...
    public static <T> List<T> getByType(AreaTypeEnum type, Function<Area, T> func) {
        return StreamUtils.convertList(areas.values(), func, area -> type.getType().equals(area.getType()));
    }
     ...
     * @return 上级区域编号
     */
    public static Integer getParentId(Integer id, @NonNull AreaTypeEnum type) {
